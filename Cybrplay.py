...
def renderer():
    ...
    #FIX: no class name issue
    start = time.perf_counter()
    raylib.InitWindow(700, 480,  b"cybrplay")
    print("init window took ", time.perf_counter() - start, " seconds")    # raylib.MaximizeWindow()
    raylib.SetAudioStreamBufferSizeDefault(4096)
    print("audio buffer set took ", time.perf_counter() - start, " seconds")    # raylib.MaximizeWindow()
    raylib.InitAudioDevice()
    print("init audio device took ", time.perf_counter() - start, " seconds")    # raylib.MaximizeWindow()
    ...
    print("load fonts took ", time.perf_counter() - start, " seconds")    # raylib.MaximizeWindow()
    raylib.SetTargetFPS(render_fps)

    visualiser = cava.start()
    print("init cava took ", time.perf_counter() - start, " seconds")    # raylib.MaximizeWindow()
    ...
    while not raylib.WindowShouldClose():
        ...
        if starting:
            ...
            print("load first song took ", time.perf_counter() - start, " seconds")    # raylib.MaximizeWindow()
            bar_width, song_x, info_x = update_layout(font, font2, title, info)
            print("layout update took ", time.perf_counter() - start, " seconds")    # raylib.MaximizeWindow()
            starting = False
